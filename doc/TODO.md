...
- [x] **9. 대화 모드(DIALOGUE) — 한 문장씩 주고받기** — DESIGN §2.7 — *완료(2026-07-06)*
  (A+B)를 한 세트로 주고받기 평탄화(`dialogueSteps`), 패턴=DRILL. 화자→voice 매핑(짝마다 남/여 무작위, 일본어+한국어 성별 일치, 단어는 나레이터). n4_dialogue(50)·n3_dialogue(50). 목록 2인 배지.
  *메모:* voice 성별은 API로 못 얻어 남성 코드 힌트로 [여,남] 선택. 정확한 통제는 mp3(§10.3).
